...
function Table(mount, doc) {
  ...
  function chrome() {
    ...
    mount.innerHTML =
      ...
      '<div class="rs-bar">' +
      '<i class="rs-bar__current" style="width:' +
      currentPct.toFixed(2) +
      '%" title="' +
      (available - counts.behind) +
      ' packages at the version the ROS index released"></i>' +
      '<i class="rs-bar__behind" style="width:' +
      behindPct.toFixed(2) +
      '%" title="' +
      counts.behind +
      ' packages older than the version the ROS index released"></i>' +
      "</div>" +
      '<p class="rs-summary__legend">' +
      '<span class="rs-key rs-key--full">' +
      counts.full +
      " on every platform</span>" +
      '<span class="rs-key rs-key--partial">' +
      counts.partial +
      " partial</span>" +
      '<span class="rs-key rs-key--missing">' +
      counts.missing +
      " not on channel</span>" +
      '<span class="rs-key rs-key--behind">of those, ' +
      counts.behind +
      ...
      filters()
        .map((f) => {
          const on = f.id === state.filter;
          return (
            '<button type="button" class="rs-filter' +
            (on ? " rs-filter--on" : "") +
            '" data-filter="' +
            f.id +
            '" aria-pressed="' +
      ...
      "<colgroup><col>" +
      active.map(() => '<col style="width:5rem">').join("") +
      ...
      "</table></div>" +
      '<div class="rs-notice rs-empty" hidden>' +
      ...
      doc.mutexes
        .map((v, i) => {
          return (
            ...
            i +
            '"' +
            (i === state.mutex ? " selected" : "") +
            ">" +
            escapeHtml(doc.mutexPackage) +
            " " +
            escapeHtml(v) +
            (i === 0 ? " (current)" : "") +
            "</option>"
  ...
  function rowHtml(row) {
    ...
    const upgrade = row.upgrade
      ? '<span class="rs-upgrade" title="' +
        ...
        ", newer than the " +
        escapeHtml(doc.mutexes[state.mutex]) +
        ' selected above">&uarr; ' +
        escapeHtml(row.upgrade.version) +
        " on " +
        escapeHtml(row.upgrade.mutex) +
    ...
    const version = row.version
      ...
      : upgrade ||
        ...
        (row.older.length
          ? '<span class="rs-elsewhere" title="Built for ' +
            escapeHtml(doc.mutexPackage) +
            " " +
            row.older.join(", ") +
            ", all older than the " +
            escapeHtml(doc.mutexes[state.mutex]) +
            ' selected above">mutex ' +
    ...
    return (
      ...
      active
        .map((p) => {
          const on = (row.mask >> p.bit) & 1;
          return (
            '<td title="' +
            p.id +
            ": " +
            (on ? "available" : "not on channel") +
            '"><span class="rs-mark rs-mark--' +
            (on ? "yes" : "no") +
            '">' +
            (on ? "✓" : "·") +
            "</span></td>"
  ...
  fetch("/data/" + distro + ".json")
    ...
    .catch((error) => {
      mount.innerHTML =
        ...
        "<p>The request failed with " +
        escapeHtml(error.message) +
        ". Browse the channel directly at " +
        '<a href="https://prefix.dev/channels/robostack-' +
        escapeHtml(distro) +
        '">prefix.dev</a>.</p></div>';
    });
}
